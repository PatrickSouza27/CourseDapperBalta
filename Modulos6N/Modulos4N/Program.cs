...
namespace Modulo4
{
    class Program
    {
        ...
        static void OneToMany(SqlConnection conectar)
        {
            string sql = @"SELECT 
                        [Categoria].[Id], [Categoria].[Name], [Curso].[IdCurso], [Curso].[Name]
                           FROM
                                [Categoria]
                        INNER JOIN [Curso] ON [Curso].[FK_Categoria] = [Categoria].[Id] ORDER BY [Categoria].[Name];";
            List<Categoria> carreira = new List<Categoria>();
            var item = conectar.Query<Categoria, Curso, Categoria>(sql, (categoria, curso) =>
            {
                var valid = carreira.Where(x=> x.Id == categoria.Id).FirstOrDefault();
                if(valid == null)
                {
                    valid = categoria;
                    valid.Cursos.Add(curso);
                    carreira.Add(valid);
                }
                else
                {
            ...
            carreira.ForEach(x =>
            {
                Console.WriteLine(x.Name); x.Cursos.ForEach(x => { Console.WriteLine(x.Name); });
            });
        }
        static void Main()
        {
